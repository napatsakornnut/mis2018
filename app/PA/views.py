...
def all_approved_pa():
    ...
    for pa in pa_query:
        if pa.round.is_closed != True and not pa.staff.is_retired:
            committee = PACommittee.query.filter_by(round=pa.round, role='ประธานกรรมการ', subordinate=pa.staff).first()
            if not committee:
                committee = PACommittee.query.filter_by(org=pa.staff.personal_info.org, role='ประธานกรรมการ',
                                                        round=pa.round).first()
    ...
    if request.method == 'POST':
        ...
        for item in all_idp_item:
            records.append({
                ...
                u'สังกัด': item.idp.staff.personal_info.org.parent if item.idp.staff.personal_info.org.parent else ''+' '+str(item.idp.staff.personal_info.org),
